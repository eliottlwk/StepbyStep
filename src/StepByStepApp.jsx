...
import {
  ...
  Calendar, Archive, RotateCcw, LogOut, Menu
} from 'lucide-react';
...
function AppShell({ user }) {
  ...
  const handleCreateTask = async () => {
    ...
    const data = {
      ...
      dueDate: form.dueDate || null, reminder: form.reminder,
      projectId: form.projectId || null, done: false, archived: false,
    };
  ...
  const handleToggleDone = (id) => {
    ...
    if (t.subtasks?.length) {
      ...
      t.subtasks.forEach(s => updateSubtask(id, s.id, { done: !allDone }));
    } else {
  ...
  const handleSaveEdit = () => {
    ...
    updateTask(editingTask.id, {
      ...
      dueDate: editingTask.dueDate || null, projectId: editingTask.projectId || null,
          ...
          <div className="task-list">
            {taskList.map(t => (
              <SortableTaskCard key={t.id} task={t} pct={getPct(t)} proj={getProject(t.projectId)}
                onToggleDone={handleToggleDone} onOpen={(id) => { setSelectedId(id); setView('detail'); setSidebarOpen(false); }} />
            ))}
          </div>
        </SortableContext>
      </DndContext>
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sbs-icon-btn:hover { background: ${EM_LIGHT}; color: ${EM_DARK}; }
    ...
    @media (max-width: 768px) {
      .sbs-overlay-bg.open { display: block; }
    }
    ...
    @media (max-width: 768px) {
      .sbs-sidebar { position: fixed; top: 56px; left: -224px; height: calc(100vh - 56px); z-index: 45; transition: left 0.25s ease; }
      .sbs-sidebar.open { left: 0; }
    }
    ...
    /* CONTENT */
    .sbs-content { flex: 1; padding: 20px; overflow-y: auto; min-width: 0; }
    ...
    /* BUTTONS */
    .btn-p { background: ${EM}; color: #fff; border: none; border-radius: 8px; padding: 9px 15px; font-size: 13.5px; font-weight: 500; cursor: pointer; display: flex; align-items: center; gap: 6px; font-family: inherit; white-space: nowrap; flex-shrink: 0; }
    ...
    @media (max-width: 400px) { .sbs-stats { grid-template-columns: repeat(3,1fr); gap: 6px; } }
    .sbs-stat { background: #fff; border: 0.5px solid #E5E7EB; border-radius: 10px; padding: 12px 14px; }
    ...
    .task-card { background: #fff; border: 0.5px solid #E5E7EB; border-radius: 11px; padding: 13px 14px; transition: border-color 0.15s; }
    ...
    @media (max-width: 480px) {
      .detail-actions { width: 100%; margin-top: 10px; justify-content: flex-start; }
    ...
    .cal-nav-btn:hover { background: ${EM_LIGHT}; color: ${EM_DARK}; }
    ...
    .cal-cell:hover { border-color: ${EM_MID}; }
    .cal-cell.empty { background: transparent; border-color: transparent; cursor: default; }
    .cal-cell.today { border-color: ${EM}; background: ${EM_LIGHT}; }
    .cal-day { font-size: 11px; font-weight: 500; color: #374151; margin-bottom: 3px; }
    .cal-cell.today .cal-day { color: ${EM_DARK}; font-weight: 700; }
    .cal-task-pill { font-size: 9px; background: ${EM_LIGHT}; color: ${EM_DARK}; border-radius: 3px; padding: 1px 3px; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    /* LOADING */
    .sbs-loading { display: flex; align-items: center; justify-content: center; height: 200px; color: #9CA3AF; font-size: 14px; gap: 10px; }
    ...
    @media (max-width: 768px) {
      ...
      .bottom-nav-btn.active { color: ${EM}; }
  ...
  if (loading) return (
    <div className="sbs"><style>{css}</style><div className="sbs-loading">Chargement...</div></div>
  );
    ...
    <div className="sbs">
      ...
      <header className="sbs-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button className="sbs-icon-btn" style={{ display: 'none' }}
            id="menu-btn"
          ...
          <div className="sbs-logo">
            ...
            <span style={{ display: 'none' }} id="logo-text">Step by Step</span>
            <style>{`@media(min-width:480px){#logo-text{display:inline!important}}`}</style>
          </div>
        </div>
      ...
      <div className="sbs-body">
        ...
        <nav className={`sbs-sidebar${sidebarOpen ? ' open' : ''}`}>
          ...
          <button className={`sbs-nav ${view === 'archive' ? 'active' : ''}`} onClick={() => navigate('archive')}>
            ...
            {archivedTasks.length > 0 && <span className="sbs-badge gray">{archivedTasks.length}</span>}
          </button>
          {reminders.length > 0 && (
          ...
          {projects.map(p => (
            <div key={p.id} className={`sbs-proj-row ${filterProject === p.id ? 'active' : ''}`}
              onClick={() => { setFilterProject(p.id); navigate('tasks'); }}>
          ...
          <button className="sbs-nav" style={{ color: EM }} onClick={() => { setShowNewProject(true); setSidebarOpen(false); }}>
            <Plus size={15} /> Nouveau projet
          </button>
          <div style={{ marginTop: 'auto' }}>
            <button className="sbs-nav" onClick={() => { openNew(); setSidebarOpen(false); }}><Plus size={16} /> Nouvelle tâche</button>
          </div>
        </nav>
        ...
        <main className="sbs-content">
          ...
          {view === 'dashboard' && <>
            <div className="sbs-ch">
              ...
              <button className="btn-p" onClick={() => { setForm(emptyForm); setShowNew(true); }}><Plus size={15} /> <span>Nouvelle</span></button>
            </div>
  ...
  ?           <EmptyState onCreateTask={() => setShowNew(true)} />
                ...
                {done.length > 0 && (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <button className="btn-s" style={{ fontSize: 12, padding: '4px 10px', gap: 4 }} onClick={() => done.forEach(t => updateTask(t.id, { archived: true }))}>
            ...
            <button className="add-btn" onClick={() => setShowNew(true)}><Plus size={16} /> Ajouter une tâche...</button>
          </>}
          ...
          {view === 'tasks' && <>
            <div className="sbs-ch">
              ...
              <button className="btn-p" onClick={() => setShowNew(true)}><Plus size={15} /> <span>Nouvelle</span></button>
            </div>
          ...
          {/* CALENDRIER */}
          {view === 'calendar' && <>
            <div className="sbs-ch"><div><div className="sbs-pt">Calendrier</div><div className="sbs-ps">Tâches par échéance</div></div><button className="btn-p" onClick={() => setShowNew(true)}><Plus size={15} /></button></div>
                ...
                return (
                  <div key={day} className={`cal-cell${isToday ? ' today' : ''}`}>
                    ...
                    {dayTasks.slice(0, 2).map(t => (
                      <div key={t.id} className={`cal-task-pill${t.priority === 'high' ? ' urgent' : ''}`}
                        onClick={() => { setSelectedId(t.id); setView('detail'); }} title={t.title}>{t.title}</div>
                    ))}
            ...
            return <>
              ...
              <div style={{ background: '#fff', border: '0.5px solid #E5E7EB', borderRadius: 12, padding: '18px 18px' }}>
                  ...
                  <div className="detail-actions">
                    {isEditing ? (
                      ...
                      <>
                        ...
                        {selected.archived
                          ? <button className="btn-s" style={{ padding: '5px 9px', color: EM }} onClick={() => updateTask(selected.id, { archived: false })}><RotateCcw size={14} /></button>
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter}
                  onDragEnd={(e) => handleSubDragEnd(selected.id, e)}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                  <input className="sbs-input" placeholder="Ajouter une étape..." value={newSubtask}
                    ...
                    onKeyDown={e => { if (e.key === 'Enter') { addSubtask(selected.id, newSubtask); setNewSubtask(''); } }} />
                  <button className="btn-p" onClick={() => { addSubtask(selected.id, newSubtask); setNewSubtask(''); }} style={{ padding: '8px 12px' }}><Plus size={15} /></button>
                </div>
              </div>
            </>;
          })()}
            ...
            {archivedTasks.length === 0
              ...
              : archivedTasks.map(t => (
                <div key={t.id} className="archive-row">
                  ...
                  <button className="btn-s" style={{ padding: '5px 10px', fontSize: 12, gap: 4, color: EM, flexShrink: 0 }} onClick={() => updateTask(t.id, { archived: false })}>
      ...
      <nav className="bottom-nav">
        ...
        <button className={`bottom-nav-btn`} onClick={() => setShowNew(true)} style={{ color: EM }}><div style={{ width: 36, height: 36, borderRadius: '50%', background: EM, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: -4 }}><Plus size={20} color="#fff" /></div></button>
      ...
      {showNewProject && (
        <div className="sbs-overlay" onClick={() => setShowNewProject(false)}>
          <div className="sbs-modal modal-sm" onClick={e => e.stopPropagation()}>
            ...
            <button className="btn-p" style={{ width: '100%', justifyContent: 'center', padding: 11 }} onClick={() => { if (!projectForm.name.trim()) return; createProject({ name: projectForm.name.trim(), color: projectForm.color }); setProjectForm({ name: '', color: '#059669' }); setShowNewProject(false); }} disabled={!projectForm.name.trim()}>Créer le projet</button>
          </div>
        </div>
      )}
...
export default function StepByStepApp() {
  ...
  if (user === undefined) return (
    <div style={{ minHeight: '100vh', background: '#F8FAF9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif', color: '#9CA3AF' }}>
      Chargement...
    </div>
  );
