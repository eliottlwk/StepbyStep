...
const CONTENT = [
  // ── CITATIONS ──
  {
    ...
    color: '#8B5CF6',
    bg: '#F5F3FF',
    text: '"La façon de commencer est de cesser de parler et de commencer à agir."',
    author: 'Walt Disney',
    cta: 'Créer ma première tâche',
  },
  {
    type: 'quote',
    icon: <Quote size={18} />,
  ...
  },
  {
    ...
    color: '#8B5CF6',
    bg: '#F5F3FF',
    text: '"Vous n\'avez pas à être parfait pour commencer, mais vous devez commencer pour être parfait."',
    author: 'Zig Ziglar',
    cta: 'Se lancer',
  },
  ...
  // ── MÉTHODES ──
  {
    ...
    subtitle: 'David Allen — Getting Things Done',
...
export default function EmptyState({ onCreateTask }) {
  ...
  const go = (dir) => {
    setAnim(false);
    setTimeout(() => {
      setIndex(i => (i + dir + CONTENT.length) % CONTENT.length);
      setAnim(true);
    }, 150);
  };
  ...
  const css = `
    ...
    .es-card { animation: fadeIn 0.3s ease; }
  `;
  ...
  return (
    <div style={{ maxWidth: 520, margin: '0 auto', padding: '8px 0' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <div style={{ width: 56, height: 56, borderRadius: 16, background: EM, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
      ...
      {/* CTA principal */}
      <button onClick={onCreateTask} style={{ width: '100%', background: EM, color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontSize: 15, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit', marginBottom: 24 }}>
        <Plus size={18} /> Créer ma première tâche
      </button>

      {/* Contenu rotatif */}
      <div className="es-card" key={index} style={{ background: item.bg, border: `0.5px solid ${item.color}22`, borderRadius: 14, padding: '20px 20px 18px', position: 'relative' }}>
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
            <button onClick={() => go(-1)} style={{ width: 26, height: 26, borderRadius: 6, border: `0.5px solid ${item.color}44`, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: item.color }}>
              <ChevronLeft size={14} />
            </button>
            <button onClick={() => go(1)} style={{ width: 26, height: 26, borderRadius: 6, border: `0.5px solid ${item.color}44`, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: item.color }}>
      ...
      <div style={{ display: 'flex', gap: 5, justifyContent: 'center', marginTop: 14 }}>
        {CONTENT.map((_, i) => (
          <div key={i} onClick={() => { setAnim(false); setTimeout(() => { setIndex(i); setAnim(true); }, 150); }}
            style={{ width: i === index ? 18 : 6, height: 6, borderRadius: 3, background: i === index ? EM : '#E5E7EB', cursor: 'pointer', transition: 'all 0.2s' }} />
        ))}
      </div>
    </div>
  );
